...
class CustomStr {
 public:
  ...
  explicit CustomStr(const char* s) {
...
class Custom {
 public:
  ...
  CustomStr operator==(const Custom& rhs) const {
    ...
    return CustomStr(
        "%g == %g && '%s' == '%s'",
        value_, rhs.value_, str().c_str(), rhs.str().c_str());
  }
...
// Define `dtype=object` types, and ensure the operations correctly reflect
// these.
struct ObjectA {};
struct ObjectB {};
...
TEST_SUBMODULE(numpy_dtype_user, m) {
  ...
  ConstructorStats::type_fallback([](py::object cls) -> std::type_index* {
    return const_cast<std::type_index*>(
        py::detail::dtype_info::find_entry(cls));
  });
  ...
  custom
    ...
    // Test referencing.
    .def("self",
         [](Custom* self) { return self; }, py::return_value_policy::reference)
    // Casting.
    // N.B. For `np.ones`, we could register a converter from `int64_t` to
    // `Custom`, but this would cause a segfault, because `np.ones` uses
    // `np.copyto(..., casting="unsafe")`, which does *not* respect
    // NPY_NEEDS_INITIALIZATION.
    ...
    // - - N.B. This shouldn't be a normal operation (upcasting?), as it may
    // result in data loss.
  ...
  // `py::vectorize` does not seem to allow custom types due to sfinae
  // constraints :(
  py::ufunc x(m, "same");
  x
