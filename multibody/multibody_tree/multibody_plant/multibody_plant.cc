...
template <typename T>
void MultibodyPlant<T>::CheckSceneGraph(
    const geometry::SceneGraph<T>* scene_graph) const {
  const bool mismatch_scene_graph =
      (scene_graph != nullptr && scene_graph != scene_graph_);
  if (!geometry_source_is_registered() || mismatch_scene_graph) {
    throw std::logic_error(
