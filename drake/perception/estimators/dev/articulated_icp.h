...
class ArticulatedIcpErrorSet {
 public:
  ...
  void resize(int num) {
    errors_.resize(Eigen::NoChange, num);
    J_errors_.resize(3 * num, num_var_);
  }
  int size() const {
    return errors_.cols();
  }
  ...
  Eigen::Ref<const Eigen::Matrix3Xd> J_error(int i) const {
    return J_errors_.middleRows(3 * i, 3);
  }
...
class ArticulatedIcpLinearizedNormCost : public ArticulatedIcpErrorCost {
 public:
  ...
  void Reset() override;
  void Add(
      const SceneState& scene_state, const ArticulatedIcpErrorSet& error_set) override;
  void Finalize() override;
...
class ArticulatedIcpBodyPoints {
 public:
   ...
   */
  void ComputeError(
      const SceneState& scene_state, ArticulatedIcpErrorSet* es) const;
 private:
 ...
 */
ArticulatedBodyInfluence IsBodyCorrespondenceInfluential(
    const Scene& scene,
    BodyIndex);
