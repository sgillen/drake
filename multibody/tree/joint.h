...
class Joint : public MultibodyTreeElement<Joint<T>, JointIndex>  {
 ...
 protected:
  ...
  struct JointImplementation {
    ...
    template <typename ToScalar>
    std::unique_ptr<typename Joint<ToScalar>::JointImplementation>
    CloneToScalar(const internal::MultibodyTree<ToScalar>& tree_clone) const {
      ...
      for (const internal::Mobilizer<T>* mobilizer : mobilizers_) {
        const internal::Mobilizer<ToScalar>* mobilizer_clone =
    ...
    /// implementation.
    std::vector<const internal::Mobilizer<T>*> mobilizers_;
