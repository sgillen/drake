...
normalize() {
    sed -r -e "s#${PWD}#\$PWD#g" -e 's#0x[0-9a-g]+#[address]#g' -e "s#${_bazel_out}#\$_bazel_out#g"
}
...
set +e
strace ./bazel-bin/bindings/pydrake/systems/issue_10255_min_repro_static 2>&1 | normalize | tee strace_static.txt
strace ./bazel-bin/bindings/pydrake/systems/issue_10255_min_repro_shared 2>&1 | normalize | tee strace_shared.txt
set -e
