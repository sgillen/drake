...
void ArticulatedIcpBodyPoints::ComputeError(const SceneState& scene_state,
  ...
  if (compute_J) {
    ...
    Eigen::Matrix3Xd meas_pts_C = tree.transformPoints(
        cache, meas_pts_W, scene.frame_W(), scene.frame_C());
    Eigen::MatrixXd J_meas_pts_W =
        tree.transformPointsJacobian(
            cache, meas_pts_C, scene.frame_C(), scene.frame_W(), false);
    Eigen::Matrix3Xd body_pts_Bi = tree.transformPoints(
        cache, body_pts_W, scene.frame_W(), frame_Bi);
    Eigen::MatrixXd J_body_pts_W =
        tree.transformPointsJacobian(
            cache, body_pts_Bi, frame_Bi, scene.frame_W(), false);
...
void ArticulatedIcpErrorNormCost::Add(const SceneState&, const ArticulatedIcpErrorSet& error_set) {
  // Get error squared.
...
void ArticulatedIcpLinearizedNormCost::Add(const SceneState& scene_state,
...
void ArticulatedIcpLinearizedNormCost::UpdateCost(solvers::QuadraticCost *cost) const {
...
void ComputeCorrespondences(const SceneState& scene_state,
  ...
  mutable_tree.collisionDetectFromPoints(
      scene_state.tree_cache(), meas_pts_W,
  ...
  for (int i = 0; i < num_points; ++i) {
    ...
    if (body_index != -1 && is_body_influential[body_index]) {
      ...
      PointCorrespondence pc{
          meas_pts_W.col(i), body_pts_W.col(i), distances[i]};
...
void ComputeCost(const SceneState& scene_state,
  ...
  cost->Reset();
  ArticulatedIcpErrorSet error_set(scene_state.scene().tree().get_num_positions());
