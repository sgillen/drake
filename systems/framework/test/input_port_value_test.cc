...
class FreestandingInputPortTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    context_.SetInputPortValue(InputPortIndex(0), std::move(free_vec));
    port0_value_ = context_.GetMutableInputPortValue(InputPortIndex(0));
    EXPECT_NE(port0_value_, nullptr);

    auto value = std::make_unique<Value<std::string>>("foo");
    auto free_value =
        std::make_unique<FreestandingInputPortValue>(std::move(value));
    context_.SetInputPortValue(InputPortIndex(1), std::move(free_value));
    port1_value_ = context_.GetMutableInputPortValue(InputPortIndex(1));
    EXPECT_NE(port1_value_, nullptr);
...
// Merely requesting mutable access to the freestanding value object should
// not cause any value change notifications; that happens only when the object's
// value is modified or mutable access to it is granted.
TEST_F(FreestandingInputPortTest, AskForMutableAccess) {
  FreestandingInputPortValue* free0 =
      context_.GetMutableInputPortValue(InputPortIndex(0));
  FreestandingInputPortValue* free1 =
      context_.GetMutableInputPortValue(InputPortIndex(1));
  EXPECT_NE(free0, nullptr);
  EXPECT_NE(free1, nullptr);
