...
class ContextBase {
 public:
  ...
  /** (Debugging) Disable caching recursively for this context and all its
  subcontexts. This forces every `Eval()` method to perform a full calculation
  rather than returning the cached one. Results should be identical with or
  without caching, except for performance. If they are not, there is either
  a problem with the specified dependencies for some calculation, or a bug in
  the caching system. */
  void SetDisableCaching(bool is_disabled) const {
    cache_.set_is_cache_disabled(is_disabled);
    const int n = do_num_subcontexts();
    for (SubsystemIndex i(0); i < n; ++i)
      do_get_subcontext(i).SetDisableCaching(is_disabled);
  }

  /** (Debugging) Returns the local name of the subsystem for which this is the
  Context. See get_system_pathname() if you want to the full name. */
