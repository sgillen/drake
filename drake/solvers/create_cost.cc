...
Binding<LinearCost> CreateLinearCost(const Expression& e) {
  ...
  return Binding<LinearCost>(c, var);
}

Binding<QuadraticConstraint> CreateQuadraticCostWithMonomialToCoeffMap(
...
// First build an Eigen vector, that contains all the bound variables.
  ...
  return CreateQuadraticCostWithMonomialToCoeffMap(monomial_to_coeff_map,
                                                   vars_vec,
                                                   map_var_to_index,
                                                   this);
}

Binding<PolynomialConstraint> CreatePolynomialCost(const Expression& e) {
  if (!e.is_polynomial()) {
    ostringstream oss;
    oss << "Expression" << e
  ...
  Binding<PolynomialConstraint> polynomial_cost(
      make_shared<PolynomialConstraint>(Vector1<Polynomiald>(polynomial),
...
Binding<Cost> CreateCost(const Expression& e) {
  if (!e.is_polynomial()) {
    ostringstream oss;
    oss << "Expression " << e
