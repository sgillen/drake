...
Vector3d ExtractJointAxis(const sdf::Model& model_spec,
  ...
  if (axis->UseParentModelFrame()) {
    ...
    const RigidTransformd X_MC =
        ToRigidTransform(model_spec.LinkByName(joint_spec.ChildLinkName())->Pose());
...
void AddJointFromSpecification(
  ...
  if (parent_body.index() == world_index()) {
    ...
    const RigidTransformd X_MP =
        ToRigidTransform(model_spec.LinkByName(joint_spec.ParentLinkName())->Pose());
